...
- **Links you can actually tap.** A link in a message opens on a tap, shortened in place so a hundred and eighty characters of tracking parameters do not fill the bubble, and a message that is only a link becomes a card: the title read off the address, the site, and a face for the kind of thing it is. There is no unfurl behind any of it — the guesswork is local arithmetic over the URL, because fetching a title or a favicon would tell a stranger's server which of you is reading what. Kin's own links are answered inside Kin: an invite opens the join screen rather than a browser tab, and a device link — which carries somebody's whole account — is drawn as a warning and refuses to be tapped at all. So is an address written to be misread: `user@host`, a look-alike alphabet, a bare IP, a plain `http:` login page.
...
- **Just me — the room you send to yourself.** Always at the top of the list, above the people: park a link to read later, jot the thing you keep forgetting, keep a photo where you can find it. Its id and its key are derived from your own private key, so every screen you use Kin on lands in the same room without anybody being told about it, and nothing is kept there that anybody else could name. It is a **kept** room, so it never expires. Your own notes are never unread and never a badge — a note left on the laptop reaches the phone as a quiet dot, because a notification about yourself is the one kind nobody wants. **🔖 Keep** on any message in any conversation puts one copy in there, labelled with where it came from, and touches nothing in the room it came out of. Sharing into Kin from another app offers Just me first, and the app icon has a *Note to self* shortcut straight into it.
...
- PWA pushed hard: manifest, icons, offline app shell, install flow, app badge, Web Push (works from the installed app, including iOS Home Screen), app shortcuts (a doodle and a note to self), and a Web Share Target so photos can be shared into Kin from any app — into Just me, offered first, as often as into a conversation.
...
The room a person keeps for themselves — **Just me** — is a group with one member in it, and the reason it is worth having rather than merely possible is the paragraph below: a phone and a laptop are the *same* member, so a room of one is the shortest path between somebody's own screens. Its id and key are derived from the identity's private key, like the sync room's, but under a different label — deliberately, because reading the sync room prunes every envelope but the newest snapshot, and notes kept there would be swept away by the next sync. It is not carried in the device snapshot either: what a screen can compute is not something to send it, and a room that is not in the list is a room no stale tombstone can take away.

Nothing there is doubled, and that falls out of two rules already in the protocol rather than a special case. The relay skips the sending *socket* rather than the sending device, so the screen that wrote a note is not handed it back while the other screen is; and the ingest path skips any envelope whose id is already on disk, which is the same rule that stops a history replay duplicating a live message. `scripts/smoke-notes.mjs` walks exactly that over two live sockets on one identity.
