...
- Roles: a **guest** can read and post but cannot invite anyone else, and a **viewer** can look but not post — the relay refuses their messages rather than trusting the client to hide the composer.
- Somebody arriving on a link can pick a name for that room alone, or go in without one and be Guest Otter.
- **Kept rooms**: a group or channel can be told to hold its messages and photos until somebody deletes them, instead of clearing itself after a week. That is what makes an album an album.
- Pairing by short code or QR is still there for adding someone stood next to you.
...
With both running, `node scripts/smoke-invite.mjs` walks the whole invite ceremony over HTTP — group, channel, link, redemption, roles, revocation — against the local relay. The unit tests drive the Durable Objects as plain classes, so this is what covers the routes, the cross-object RPC and the real request signing agreeing with one another.
